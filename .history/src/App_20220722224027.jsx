...
function Home() {
  ...
  useEffect(() => {
    console.log('click')
    if (startRef && endRef && flyRef && flyRef?.current) {
      // beforeRun()
      console.log('beforerun')
      let isRunning = false
      const runTime = 500
      const flyOuterRef = flyRef?.current?.flyOuterRef.current
      const flyInnerRef = flyRef?.current?.flyInnerRef.current
      // 现在起点距离终点的距离
